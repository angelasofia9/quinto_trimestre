...
async function agregarEstudianteDesdeModal() {
  try {
    ...
    if (nombre && !isNaN(nota1) && !isNaN(nota2) && !isNaN(nota3)) {
      await agregarEstudiante(nombre, nota1, nota2, nota3, definitiva);
      cerrarModal();
...
async function agregarEstudiante(nombre, nota1, nota2, nota3, definitiva) {
...
async function guardarEstudianteEditado() {
  try {
    ...
    if (nombre && !isNaN(nota1) && !isNaN(nota2) && !isNaN(nota3)) {
      await actualizarEstudiante(id, nombre, nota1, nota2, nota3, definitiva);
      cerrarModalEdit();
    } else {
      throw new Error("Por favor, ingresa datos válidos.");
    }
...
async function actualizarEstudiante(id, nombre, nota1, nota2, nota3, definitiva) {
  const tabla = document.getElementById("tablaEstudiantes");
...
async function eliminarEstudiante(boton) {
  try {
    const fila = boton.parentNode.parentNode;
    const id = fila.cells[0].innerHTML;
